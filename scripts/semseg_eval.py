...
Supports two modes:
1. Dataset GT mode (load_semseg=True): Uses dataset's semantic segmentation ground truth
2. Open-vocab mode (load_semseg=False): Uses encoder's class list for open-vocabulary prediction
...
class SemSegEvalConfig:
  ...
  # Export per-class point clouds as PLY files
  export_class_plys: bool = False

  # Open-vocabulary mode: Define classes to recognize (used when load_semseg=False)
  # This provides a direct way to specify classes in config without modifying encoder config
  open_vocab_classes: Tuple[str] = tuple()
...
class SemSegEval:
  def __init__(self, cfg):
    ...
    # ========================================================================
    # NEW: Support two modes - Dataset GT mode vs Open-vocab mode
    # ========================================================================
    self.use_dataset_gt = getattr(cfg.dataset, "load_semseg", False)
    
    if self.use_dataset_gt:
      # ===== Mode 1: Dataset GT mode (original behavior) =====
      logger.info("[SemSegEval] Using DATASET semantic classes (GT mode)")
      ...
      # Use dataset's class mapping directly (keep as list for compatibility)
      self._cat_index_to_cat_name = self.dataset._cat_index_to_cat_name
    else:
      # ===== Mode 2: Open-vocab mode (new behavior) =====
      logger.info("[SemSegEval] Using ENCODER semantic classes (open-vocab mode)")
      
      # Priority 1: Use open_vocab_classes from config (NEW!)
      if len(cfg.open_vocab_classes) > 0:
        class_names = list(cfg.open_vocab_classes)
        logger.info("Using open_vocab_classes from config: %s", class_names)
      # Priority 2: Use encoder.classes (fallback for backward compatibility)
      elif hasattr(cfg.encoder, "classes") and len(cfg.encoder.classes) > 0:
        class_names = list(cfg.encoder.classes)
        logger.info("Using encoder.classes: %s", class_names)
      else:
        raise ValueError(
          "load_semseg=False but neither 'open_vocab_classes' nor 'encoder.classes' "
          "is provided in config. Please specify classes to recognize."
        )
      
      # Create a list structure matching the GT mode format
      self._cat_index_to_cat_name = class_names
      self.num_classes = len(class_names)

    # Cache commonly used class indices
    self.soil_class_index = None
    for idx, name in enumerate(self._cat_index_to_cat_name):
      if name is None:
        continue
      if name.strip().lower() in ["soil pile", "soil"]:
    ...
    if hasattr(cfg, "vis") and cfg.vis is not None:
      ...
      # Use the unified _cat_index_to_cat_name for annotation context
      rr.log("/", rr.AnnotationContext(
        rr.AnnotationInfo(id=x, label=y) for x, y
        in enumerate(self._cat_index_to_cat_name)
      ))
    ...
    if (hasattr(cfg.querying, 'auto_generate_query_file') and 
      cfg.querying.auto_generate_query_file):
      self._generate_query_file()

  def _generate_query_file(self):
    query_file_path = self.cfg.querying.query_file
    
    if len(self.cfg.open_vocab_classes) > 0:
        classes = list(self.cfg.open_vocab_classes)
    elif hasattr(self.cfg.encoder, "classes"):
        classes = list(self.cfg.encoder.classes)
    else:
        logger.warning("No classes found for query file generation")
        return
    ...
    logger.info(f"Generated query file with {len(classes)} classes: {query_file_path}")
  ...
  def compute_semseg_gt(self):
    ...
    """
    if not self.use_dataset_gt:
      logger.warning("compute_semseg_gt called in open-vocab mode - no GT available")
      return None, None
  ...
  def compute_text_embeds(self):
    ...
    # NEW: Use unified _cat_index_to_cat_name (list format)
    if self.use_dataset_gt:
      # In GT mode, skip the ignore class (index 0)
      names = [self._cat_index_to_cat_name[i] for i in range(1, self.num_classes)]
    else:
      # In open-vocab mode, use all classes
      names = self._cat_index_to_cat_name
  ...
  def predict_and_semseg_eval(self, semseg_gt_xyz, semseg_gt_label,
    ...
    # Only compute metrics if GT is available
    m = None
    if semseg_gt_xyz is not None and semseg_gt_label is not None:
      m = self.compute_semseg_metrics(
        semseg_gt_label, semseg_pred_label, semseg_gt_xyz, semseg_pred_xyz)

    # Visualization
    if self.vis is not None:
      # Log all predictions together
      self.vis.log_label_pc(semseg_pred_xyz, semseg_pred_label,
                            layer="predictions/all_voxels")
      
      # 🔑 New: Log each class separately
      offset = 1 if not self.use_dataset_gt else 0
      for class_id, class_name in enumerate(self._cat_index_to_cat_name):
        if class_name is None:
          continue
        
        pred_value = class_id + offset
        class_mask = semseg_pred_label == pred_value
        
        if not torch.any(class_mask):
          logger.debug(f"No predictions for class '{class_name}'")
          continue
        
        # Create separate layer for each class
        class_xyz = semseg_pred_xyz[class_mask]
        class_label = semseg_pred_label[class_mask]
        layer_name = f"predictions/classes/{class_name}"
        
        self.vis.log_label_pc(class_xyz, class_label, layer=layer_name)
        logger.info(f"Logged {torch.sum(class_mask).item()} voxels for class '{class_name}'")
      
      # Soil-only visualization (if available)
      if self.soil_class_index is not None:
        soil_pred_value = self.soil_class_index + offset
        soil_mask = semseg_pred_label == soil_pred_value
        if torch.any(soil_mask):
          self.vis.log_label_pc(semseg_pred_xyz[soil_mask],
                                semseg_pred_label[soil_mask],
                                layer="predictions/soil_highlight")
  ...
  def log_metrics(self, i, m: dict[str, Union[float, torch.Tensor]], 
    ...
    if self.vis is not None:
      ...
      start_idx = 1 if self.use_dataset_gt else 0
      for i, cls in enumerate(self._cat_index_to_cat_name[start_idx:]):
        cls = cls.replace(" ", "_")
        for met in class_wise_metrics:
          rr.log(f"metrics/{prefix}/classwise/{i+start_idx}_{cls}/{met}",
  ...
  def export_class_voxels_ply(self, pred_xyz: torch.FloatTensor,
                              pred_label: torch.LongTensor):
    """Export per-class point clouds as PLY files.
    ...
    Args:
      ...
      pred_label: N long tensor of predicted class labels
    """
    logger.info("Exporting per-class PLY files...")
    
    # Get unique prediction values to see what we actually have
    unique_preds = torch.unique(pred_label).cpu().tolist()
    logger.info(f"Unique prediction values: {unique_preds}")
    
    # In open-vocab mode, predictions are offset by +1 (0 is reserved for ignore)
    # So we need to map: prediction 1 → class 0, prediction 2 → class 1, etc.
    offset = 1 if not self.use_dataset_gt else 0
    ...
    for class_id, class_name in enumerate(self._cat_index_to_cat_name):
      ...
      # Map class_id to prediction value
      pred_value = class_id + offset
      
      # Filter for this class
      class_mask = pred_label == pred_value
      if not torch.any(class_mask):
        logger.warning(f"No voxels found for class '{class_name}' (pred_value={pred_value})")
        continue
      
      class_xyz = pred_xyz[class_mask].cpu().numpy()
      num_points = class_xyz.shape[0]
      
      # Create filename
      safe_name = class_name.replace(" ", "_").replace("/", "_")
      output_path = os.path.join(
        self.cache_scene_dir,
        f"class_{class_id}_{safe_name}.ply"
      )
      
      logger.info(f"Saving {num_points} voxels for class '{class_name}' to {output_path}")
      
      # Write PLY file (simple version without color)
      with open(output_path, 'w') as f:
        f.write("ply\n")
        f.write("format ascii 1.0\n")
        f.write(f"element vertex {num_points}\n")
  ...
  def save_metrics(self,
    ...
    # Skip metric saving if no metrics available (open-vocab mode without GT)
    if last_m is None:
      logger.info("No metrics to save (open-vocab mode without GT)")
      return
    ...
    for i in range(start_idx, self.num_classes):
      fields = [
        str(i),
        str(self._cat_index_to_cat_name[i]),
      ]
      fields.extend([str(last_m[k][i-start_idx].item()) for k in class_wise_metrics])
    ...
    for i in range(start_idx, self.num_classes):
      ...
      for step, mm in m.items():
        if mm is None:
          continue
        fields = [str(step)]
        fields += [str(mm[k][i-start_idx].item()) for k in class_wise_metrics]
        csv_lines.append(",".join(fields) + "\n")

      results_file_name = os.path.join(
        results_dir,
        f"{i}_{self._cat_index_to_cat_name[i]}.csv")
      with open(results_file_name, "w") as f:
        f.writelines(csv_lines)

  def run(self):
    ## 1. Get Semantic Segmentation Ground Truth (only in GT mode)
    semseg_gt_xyz = None
    semseg_gt_label = None
    ...
    if self.use_dataset_gt:
      ...
      logger.info("Running in open-vocab mode - no ground truth available")
    ...
    if self.cache_is_valid("map") and self.cfg.online_eval_period <= 0:
      ...
      # Set _cat_index_to_cat_name on dataset for compatibility
      if not self.use_dataset_gt:
        # In open-vocab mode, use our class list
        self.dataset._cat_index_to_cat_name = self._cat_index_to_cat_name
      else:
        # In GT mode, use dataset's original mapping
    ...
    if self.store_output and hasattr(self, 'last_pred_xyz') and hasattr(self, 'last_pred_label'):
      ...
      # Save per-class PLY files if requested
      if self.cfg.export_class_plys:
        self.export_class_voxels_ply(self.last_pred_xyz, self.last_pred_label)
